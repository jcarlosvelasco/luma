...
let DEFAULT_MODEL = ModelInfo.ofaSmall
let DEFAULT_PROMPT = "Labrador in the style of Vermeer"

enum GenerationState {
    case startup
    case running(StableDiffusionProgress?)
    case complete(String, CGImage?, UInt32)
    case userCanceled
    case failed(Error)
}
...
class GenerationContext: ObservableObject {
    ...
    func generate(startingImage: CGImage? = nil) async throws -> GenerationResult {
